...
| **TC_ADMIN_6** | Verify Immutable Audit Log | Change a child's height parameter; log in as Admin, view `Audit Logs` page. | Log record is created; details the User ID, action (Update), old values, and new values. | Displays log: "User updated height of Child ID 4 from 88cm to 90cm" along with IP address. | **PASSED** |
| **TC_ADMIN_7** | Review and Approve GAD Event | Admin reviews proposal budget, clicks "Approve". | Proposal state updates to "Approved"; GAD budget allocation is verified. | Event is published on public feed; budget is tracked. | **PASSED** |
...
| **TC_SEC_4** | Issue BPO with 15-day SLA | Select Critical Case, click "Issue BPO", verify expiration date. | Database updates BPO status to Active; sets BPO expiration date to precisely Current Date + 15 Days. | Sets BPO status indicator to "Active"; starts a daily countdown tracker in the panel. | **PASSED** |
...
| **TC_SEC_10** | Tag GAD Beneficiary | Secretary navigates to Members tab, selects a resident, clicks "Tag GAD Beneficiary" for a specific GAD program. | System assigns resident to the GAD participant database. | User's record displays their active GAD beneficiary allocation details. | **PASSED** |
| **TC_SEC_11** | Claim Dispatch benefit | Secretary clicks "Claim Benefit" under the beneficiary dispatch logs. | State updates to "Claimed" and records the transaction date and user signature. | Renders transaction receipt; logs status as Claimed on the list. | **PASSED** |
...
| **TC_PRES_2** | Create GAD Event Proposal | Organization President fills out event title, proposal details, budget, and submits. | Proposal saved; status set to "Pending"; notification sent to system Admin. | Proposal appears in admin review list; president receives pending status indicator. | **PASSED** |
...
| **TC_CIT_3** | Register for GAD Event | Public user views event details, clicks "Register", inputs email. | Checks vacancy; adds resident to event participant database. | Displays registration confirmation; sends calendar details via queue worker. | **PASSED** |
