    ...
    return (
        <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
            <DialogContent className="sm:max-w-lg max-h-[90vh] flex flex-col p-0 overflow-hidden">
                ...
                {/* Body Content (Proportional spacing, zero text bloat, no nested box allowance) */}
                <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2.5">
                    {/* Compact Applicant Info Strip */}
                    ...
                    {/* Box 1: Officer Disapproval Reason (Single-layer card, tight allowance) */}
                    ...
                    {/* Box 2: Resident Appeal Statement (Single-layer card, tight allowance) */}
                    <div className="rounded-lg border border-amber-200/80 dark:border-amber-900/50 bg-amber-50/30 dark:bg-amber-950/20 p-3 space-y-1.5">
                        ...
                        {/* Supporting Documents (Only shown if files exist) */}
                    ...
                    {/* Resolved Summary (If historical) */}
                    {appeal.actioned_at && (
                        <div className="p-2.5 rounded-lg bg-muted/50 border text-xs flex items-center justify-between">
                            <span className="flex items-center gap-1.5 font-medium">
                                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                                Resolved by: <strong className="text-foreground">{appeal.approved_by || 'Executive Council'}</strong>
                            </span>
                ...
                {/* Footer (Clean, properly sized buttons without overflow) */}
                <DialogFooter className="px-4 py-3 border-t bg-muted/20 shrink-0 flex items-center justify-between sm:justify-between w-full">
                    ...
                    {!isHistoryTab && (appeal.status === 'appealed' || appeal.status === 'Appealed' || appeal.status === 'disapproved' || appeal.status === 'rejected') && (
                        <div className="flex items-center gap-2">
                            <Button
                                type="button"
                                variant="outline"
                                onClick={() => onSustainClick(appeal)}
                                className="h-9 px-3.5 text-xs sm:text-sm font-bold text-rose-700 dark:text-rose-300 border-rose-300 hover:bg-rose-50 dark:hover:bg-rose-950/40 gap-1.5"
                            >
